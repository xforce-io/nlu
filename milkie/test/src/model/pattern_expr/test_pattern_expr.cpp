...
void testBugfix() {
  std::wstring expr = Helper::PreprocessExprLine(L"{ #Chk(_cont_np_) #Chk(_cont_np_) -> syntactic._cont_np_ }");
  auto ret = PatternExpr::Build(milkie->GetReferManager(), kTestBlockKey, expr);
  ASSERT_TRUE(ret.first != nullptr);

  std::wstring query = L"上海和广西";
  auto context = std::make_shared<Context>(query);
  auto &nluContext = *(context->GetSentence().GetNluContext());

  FragmentSet<Chunk> chunks(query);
  chunks.Add(Chunk(nluContext, SyntaxTag::Type::kContNp, 0, 2));
  chunks.Add(Chunk(nluContext, SyntaxTag::Type::kC, 2, 1));
  chunks.Add(Chunk(nluContext, SyntaxTag::Type::kContNp, 3, 2));
